...
void SPI_Init(void)
{
    ...
    SPI_WRITE_TX0(SPI0, 0xFFFFFFFF);    /* Dummy Write to prevent TX under run */
}
...
void GPIOP0P1_IRQHandler(void)
{
    ...
    if(GPIO_GET_INT_FLAG(P0, BIT4))
    {
        ...
        while(P04 == 0)
        {
            ...
            {
                SPI_WRITE_TX0(SPI0, _response_buff[g_u32TxDataCount]);    /* Write to TX FIFO */
                g_u32TxDataCount++;
            ...
            if(SPI_GET_RX_FIFO_EMPTY_FLAG(SPI0) == 0)
            {
                ...
                spi_rcvbuf[g_u32RxDataCount++] = SPI_READ_RX0(SPI0);    /* Read RX FIFO */
            }
        ...
        if(P04 == 1)
        {
            ...
            {
                SPI_WRITE_TX0(SPI0, 0xFFFFFFFF);    /* Write to TX FIFO */
            }
        }
    }
    else
