...
void GPIOP0P1_IRQHandler(void)
{
    /* To check if P1.0 interrupt occurred */
    if(GPIO_GET_INT_FLAG(P1, BIT0))
    {
...
int32_t main(void)
{
    ...
    printf("\n\nCPU @ %d Hz\n", SystemCoreClock);
    printf("+-------------------------------------------------------------------+\n");
    printf("|  SYS_PowerDown_MinCurrent and Wake-up by P1.0 Sample Code         |\n");
    printf("+-------------------------------------------------------------------+\n\n");
    ...
    printf("|  6. Wait for P1.0 falling-edge interrupt event to wake-up the MCU |\n");
    printf("+-------------------------------------------------------------------+\n\n");

    /*
        To measure Power-down current:
        On NuTiny-SDK-M058SFAN V1.0 board, remove components, e.g. Nu-Link-Me, PR and R11.
    */
    ...
    /* Configure P1.0 as Quasi mode and enable interrupt by falling edge trigger */
    P1->PMD = (P1->PMD & (~GPIO_PMD_PMD0_Msk)) | (GPIO_PMD_QUASI << GPIO_PMD_PMD0_Pos);
    P1->IMD |= (GPIO_IMD_EDGE << 0);
    P1->IEN |= (BIT0 << GPIO_IEN_IF_EN_Pos);
    ...
    /* Waiting for P1.0 falling-edge interrupt event */
