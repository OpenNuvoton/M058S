...
void LIN_TestItem()
{
    ...
    printf("|                                                           |\n");
    printf("| The sample code needs to enable semihosted.               |\n");
    printf("| To measure UART0 TXD pin (P3.1) to check waveform ...     |\n");
    printf("+-----------------------------------------------------------+\n");
    printf("| Quit                                              - [ESC] |\n");
    printf("+-----------------------------------------------------------+\n\n");
}
...
void LIN_FunctionTest()
{
    ...
    /*
        Please enable semihosted to show messages on debug session.
        Keil users must define DEBUG_ENABLE_SEMIHOST in both C/C++ and Asm preprocessor symbols.
...
void LIN_SendHeader(uint32_t u32id)
{
    ...
    UART0->ALT_CSR &= ~(UART_ALT_CSR_LIN_TX_EN_Msk | UART_ALT_CSR_LIN_TX_EN_Msk | UART_ALT_CSR_UA_LIN_BKFL_Msk);
    UART0->ALT_CSR |= (UART_ALT_CSR_LIN_TX_EN_Msk | (11 << UART_ALT_CSR_UA_LIN_BKFL_Pos));
